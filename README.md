...
Claude Code 用の `review` / `review-fix-loop` / `review-iterate` は Codex MCP の利用を前提としていたため廃止（Codex を利用できない sbx 環境が主になったため）。Codex 版 `review` も廃止済み。レビューは Claude Code 組み込みの `/code-review` 等を利用する。
